...
def get_connection_redis():
    redisURL = os.environ.get("CKAN_REDIS_URL", "")
    if redisURL == "":
        redisURL = os.environ.get("CKAN__REDIS__URL", "")

    _connection_pool = None
    if not config.get('ckan.harvest.mq.hostname') and redisURL != "":
        if '@' in redisURL:
            # Specific for Azure - ConnectionPool.from_url(url) fails with the url
            urlNoRedis = redisURL.split('redis://')[1]
            PASSWORD = urlNoRedis.split('@')[0]
